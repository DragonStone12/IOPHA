...
const generateReleaseNotes = async () => {
    ...
    commits.forEach(commit => {
        ...
        const issueNumbers = extractIssueNumbers(`${commit.subject} ${commit.body || ''}`);
        
        // Build issue links string
        const issueLinks = issueNumbers.length > 0 
    ...
    markdownContent += `*Release Date: ${new Date().toISOString().split('T')[0]}*\n\n`;
