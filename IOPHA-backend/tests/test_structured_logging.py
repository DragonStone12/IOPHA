...
from app.logging import CentralizedLoggingMiddleware, JsonTelemetryFormatter, PathSanitizer
...
class TestJsonTelemetryFormatter:
    ...
    def test_includes_required_fields(self):
        formatter = JsonTelemetryFormatter()
        record = logging.LogRecord(
            name="com.example.PatientService", level=logging.INFO, pathname="", lineno=0,
            msg="request.start", args=(), exc_info=None
        )
...
class TestPathSanitizer:
    ...
    def test_sanitizes_multiple_segments(self):
        assert PathSanitizer.sanitize_path("/patients/123/records/456") == "/patients/:id/records/456"

    def test_redacts_sensitive_query_params(self):
        query = {"email": "test@example.com", "phone": "555-123-4567", "specialty": "cardiology"}
...
class TestCentralizedLoggingMiddleware:
    ...
    def test_logs_request_start_and_complete(self, client, caplog):
        ...
        messages = [r.getMessage() for r in caplog.records if r.name == "com.example.PatientService"]
    ...
    def test_request_log_contains_method_and_user_agent(self, client, caplog):
        with caplog.at_level(logging.INFO, logger="com.example.PatientService"):
            client.get("/health", headers={"X-Request-ID": "req-1", "User-Agent": "test-agent"})
        records = [r for r in caplog.records if r.getMessage() == "request.start"]
        extra = records[0].extra_context
